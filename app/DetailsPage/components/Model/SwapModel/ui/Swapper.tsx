...
const fetchAllowance = async (tokenIn: Token, recipient: Address) => {
  return thirdwebAllowance({
    contract: getContract({ address: tokenIn.address }),
    owner: recipient,
    spender: ROUTER,
  });
};
...
function SwapButton({
  ...
  const refetchAllowance = useCallback(
    () => fetchAllowance(tokenIn, recipient).then(setAllowance),
    [tokenIn, recipient]
  );
  const refetchBalance = useCallback(
    () => fetchBalance(tokenIn, recipient).then(setBalance),
    [tokenIn, recipient]
  );
  useEffect(() => {
    refetchAllowance();
...
export default function Swapper() {
  ...
  const inputToken = useMemo(
    () => (inputTokenKey ? tokens[inputTokenKey] : undefined),
    [inputTokenKey]
  );
  const outputToken = useMemo(
    () => (outputTokenKey ? tokens[outputTokenKey] : undefined),
    [outputTokenKey]
  );
  const {
  ...
  const canSwap =
    !quoteLoading && account && inputToken && outputToken && amount && fee;
              ...
              {quoteLoading ? (
                ...
                <div>
                  {outputAmount && outputToken
                    ? toTokens(outputAmount, outputToken.decimals)
                    : 0}
                </div>
              )}
            </div>
